...
public class TransferRequestDTO implements Validator {
    ...
    public void validate(Object target, Errors errors) {
        ...
        String transferFeeStr = transferRequestDTO.getFees();
        ...
        if (transferFeeStr.length() == 0 || transferFeeStr.length() > 2) {
            errors.rejectValue("fees", "fees.length");
        } else {
            Long fee = null;
            try {
                fee = Long.parseLong(transferFeeStr);
            } catch (Exception e) {
                errors.rejectValue("fees", "fees.format");
            }
            if (fee != null) {
                if (fee > 50L || fee < 0) errors.rejectValue("fees", "fee.range");
            }
        }
    }
}
